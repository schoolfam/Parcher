...
class AttendanceAdapter: RecyclerView.Adapter<AttendanceAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        val count:Int = attendance.filter { attendance -> attendance.studentId==student.id}.size
            ...
            when(it.id){
                ...
                R.id.fab_absent_attendance_button -> {
                    val date = SimpleDateFormat("dd/MM/yyyy").parse(date)
                        val attendance = attendance.find { attendance -> attendance.studentId == student.id }
