...
class AddTeachersFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_add_teachers, container, false)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        teacherRegisterButton.setOnClickListener {
            if (teacherFirstNameEditText.text.toString().startsWith(" ")||teacherFirstNameEditText.text.toString()==""||
                ...
                val gender:String = if(teacherMaleRadioButton.isChecked){
                    "Male"
                } else{
                    "Female"
                }
                ...
                val newUser = User(
                    ...
                    Toast.makeText(activity,"Progress ",Toast.LENGTH_LONG).show()
                        ...
                        val success = userViewModel.insertUser(newUser)
                        Toast.makeText(activity,"User id: "+success,Toast.LENGTH_LONG).show()
                        if(success!=-1L){
                            val newTeacher = Teacher(success,sectionId)
                            ...
                            findNavController().navigate(R.id.action_global_addTeachersFragment)

                        }
                        else{
                            Snackbar.make(it, "Not Connected To The Server", Snackbar.LENGTH_LONG)
                                .setAction("Action", null).show()
                        }
...
//                Toast.makeText(activity,"Section Id: "+it[0].section_id,Toast.LENGTH_LONG).show()
//                Toast.makeText(activity,"teachers: "+it.size,Toast.LENGTH_LONG).show()
                    if (it.size>0){
                        var counter = 0
                        it.forEach{
                            if(it.section_id == section.id){
                                counter++
                            }
                        }
